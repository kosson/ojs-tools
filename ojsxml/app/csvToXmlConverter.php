...
class csvToXmlConverter {
    ...
    private function generateIssuesXml($sourceDir, $destinationDir) {
        ...
        for ($i = 0; $i < ceil($issueCount / Config::get("issues_per_file") ); $i++) {
            // Get issue data to create dynamic filename
            $issuesData = $dbManager->getIssuesData($i);
            
            if (!empty($issuesData)) {
                $firstIssue = $issuesData[0];
                // Create filename from issueTitle-year-volume-issue
                $filenameParts = [];
                
                // Sanitize issue title - replace spaces and special chars with hyphens
                $issueTitle = preg_replace('/[^a-zA-Z0-9]+/', '-', $firstIssue['issueTitle']);
                $issueTitle = trim($issueTitle, '-');
                $filenameParts[] = $issueTitle;
                
                if (!empty($firstIssue['year'])) {
                    $filenameParts[] = $firstIssue['year'];
                }
                if (!empty($firstIssue['volume'])) {
                    $filenameParts[] = 'vol' . $firstIssue['volume'];
                }
                if (!empty($firstIssue['issue'])) {
                    $filenameParts[] = 'iss' . $firstIssue['issue'];
                }
                
                $fileName = implode('-', $filenameParts) . '.xml';
            } else {
                // Fallback to old naming if no data available
            ...
            // Store the filename for later use (last one will be stored)
            file_put_contents($destinationDir . '/.last_xml_filename', $fileName);
        }
        Logger::print("----------------------------------------");
